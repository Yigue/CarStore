...
public class AdminBackfillEndpointTests
{
    ...
    private static async Task SeedCarWithLegacyImage(CustomWebApplicationFactory factory)
    {
        ...
        // Entity.Id is generated by EF on Add (not in the constructor), so Marca must be
        // tracked before the Modelo is built — otherwise modelo.MarcaId captures Guid.Empty
        // and the FK to Marca fails under SQLite (which enforces FKs).
